...
if (!process.env.ANTHROPIC_API_KEY) {
  console.warn("警告: ANTHROPIC_API_KEY が設定されていません。.envファイルを確認してください。");
}
...
- 各日程の最後に「---」区切り線を入れてください`;
  ...
  try {
    ...
    if (error.status === 401) {
      res.status(500).json({
        error:
          "APIキーが無効です。.envファイルのANTHROPIC_API_KEYを確認してください。",
      });
    } else if (error.status === 429) {
      res.status(429).json({
        error:
          "APIのリクエスト制限に達しました。しばらく待ってから再試行してください。",
      });
    } else {
      res.status(500).json({
        error:
          "旅行プランの生成中にエラーが発生しました。しばらく待ってから再試行してください。",
      });
    }
  }
});
...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", message: "AI旅行プランナーAPI稼働中" });
});

app.listen(PORT, () => {
  console.log(`サーバー起動: http://localhost:${PORT}`);
  console.log(`APIキー設定: ${process.env.ANTHROPIC_API_KEY ? "済" : "未設定"}`);
});
